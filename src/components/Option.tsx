import { InputHTMLAttributes, SyntheticEvent, useEffect, useState } from "react";
...
const Option: React.FC<OptionProps> = ({ className, options, ...props}) => {
    ...
    useEffect(() => {
        setOptionValues(options.filter((item) => item !== currentOption).map((item: string) => { return(
        <OptionUnit key={item} value={item} onClick={handleOptionClick(item)}></OptionUnit>
    )}))
    ...
    return (
        ...
        <div className="border-borderColor bg-transparent border-2 px-3 py-1 rounded-md hover:bg-modalBackground text-secondary relative h-full">
            <div className="flex gap-3 items-center justify-between h-full">
                ...
                <img src={dropdownArrow} className={`w-5 ${isOpened && 'rotate-180'}`}></img>
            </div>
            <input 
