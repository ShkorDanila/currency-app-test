...
export const formatCost = (cost: string) => {
  ...
  if (item / BILLION > 1) {
    text += (Number(item) / BILLION).toFixed(2).toString() + "b";
    return text;
  }
  if (item / MILLION > 1) {
    text += (Number(item) / MILLION).toFixed(2).toString() + "m";
    return text;
  }
  if (item / THOUSAND > 1) {
    text += (Number(item) / THOUSAND).toFixed(2).toString() + "k";
    return text;
  }
  if (item <= 10 / BILLION) {
    text += (Number(item) * BILLION).toFixed(2).toString() + "e-9";
    return text;
  }
  if (item <= 10 / MILLION) {
    text += (Number(item) * THOUSAND).toFixed(2).toString() + "e-6";
    return text;
  }
  if (item <= 10 / THOUSAND) {
    text += (Number(item) * THOUSAND).toFixed(2).toString() + "e-3";
    return text;
  }
