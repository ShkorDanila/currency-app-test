...
export default function AssetsPortfolio() {
  const portfolioCoins = useSelector(
    (state: IRootState) => state.portfolioReducer.items
  );
  const currentPortfolio = useSelector(
    (state: IRootState) => state.currentPortfolioReducer.items
  );
  ...
  return (
    <>
      <section
        ...
        className=' p-3 rounded-md border-borderColor border-[2px]  cursor-pointer'
      >
        <div>
          ...
          <Text>
            {formatCost(
              portfolioCoins
                .reduce(
                  (accum: number, coin: CoinTypeStrict) =>
                    accum + Number(coin.priceUsd) * coin.count,
                  0
                )
                .toString()
            )}{" "}
          ...
          <Text>
            {getPercent(
              portfolioCoins.map((item) => Number(item.priceUsd)),
              currentPortfolio.map((item) => Number(item.priceUsd))
            )}
          </Text>
        </div>
      </section>
