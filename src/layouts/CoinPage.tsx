...
export default function CoinPage() {
  ...
  const coinQuery = useQuery({
    queryKey: ["coin"],
    queryFn: () => getCurrentCoin(coinId?.toString() || "bitcoin"),
  });

  const chartQuery = useQuery({
    queryKey: ["chart-data"],
    queryFn: () =>
      getCurrentCoinHistory(
        coinId?.toString() || "",
        Date.now() - currentTimeMode.timeoffset,
        Date.now(),
        currentTimeMode.tag
      ),
  });
  ...
  useEffect(() => {
    ...
    if (chartQuery.data) {
      setCurrentChartConfig(
        getChartConfig(
          chartQuery.data.map((item: ChartProps) =>
            new Date(item.time).toLocaleString("ru-RU", {
              timeZone: "Europe/Moscow",
            })
          ),
          chartQuery.data.map((item: ChartProps) =>
            Number(Number(item.priceUsd).toFixed(2))
          )
        )
      );
    }
  ...
  if (coinQuery.isPending)
    return (
      <Loader isVisible={coinQuery.isPending}>
        <img
  ...
  if (coinQuery.error && !coinQuery.data)
    return <Text>An error has occurred: {coinQuery.error.message}</Text>;
  ...
  return (
    <>
      <div className=' flex items-center h-full justify-evenly relative flex-col md:flex-row'>
        <div className=' flex flex-col items-start gap-3 p-6'>
          <div className=' flex items-center gap-5'>
            <img
              src={`https://assets.coincap.io/assets/icons/${coinQuery.data.symbol.toLocaleLowerCase()}@2x.png`}
              className=' w-9'
            ></img>
            <Text>{coinQuery.data.name}</Text>
            <Text variant='utility'>{coinQuery.data.symbol}</Text>
          </div>
          <Text variant='utility'>
            rank:{" "}
            {coinQuery.data.rank
              ? formatCost(coinQuery.data.rank)
              : "not found"}
          </Text>
          <Text variant='utility'>
            supply:{" "}
            {coinQuery.data.supply
              ? formatCost(coinQuery.data.supply)
              : "not found"}
          </Text>
          <Text variant='utility'>
            price:{" "}
            {coinQuery.data.priceUsd
              ? formatCost(coinQuery.data.priceUsd)
              : "not found"}
          </Text>
          <Text variant='utility'>
            capitalisation:{" "}
            {coinQuery.data.marketCapUsd
              ? formatCost(coinQuery.data.marketCapUsd)
              : "not found"}
          </Text>
          <Text variant='utility'>
            max supply:{" "}
            {coinQuery.data.maxSupply
              ? formatCost(coinQuery.data.maxSupply)
              : "not found"}
          </Text>
          <Button onClick={handleCoinModalClick}>
        ...
        <div className=' flex flex-col items-center overflow-hidden'>
          ...
          <div className=' flex items-center gap-3'>
            <Button onClick={handleHourClick}>
              <Text variant={"utility"} id='oneHourBtn'>
                1 HOUR
              </Text>
            </Button>
            <Button onClick={handleBiHourClick}>
              <Text variant={"utility"} id='twHourBtn'>
                12 HOURS
              </Text>
            </Button>
            <Button onClick={handleDayClick}>
              <Text variant={"utility"} id='oneDayBtn'>
                1 DAY
              </Text>
            </Button>
          </div>
      ...
      <CoinModal
        ...
        onModalClick={handleCoinModalClick}
        coin={coinQuery.data}
      ></CoinModal>
    </>
  );
